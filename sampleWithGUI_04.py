...
IMAGE_PATH5 = "./megane/AUCF-22A-165_1023.png"  # 画像パス
...
layout = [
		  ...
		  [sg.Text('ランドマーク', size=(15, 1)), sg.Combo(('あり', 'なし'), default_value='なし', size=(5, 1), key='landmark'),
		  		  sg.Text('メガネ表示', size=(10, 1)), sg.Combo(('AUCF-22A-181_86', 'AMTF-17A-047_97', 'AMCF-22A-229_127', 'AUMF-22A-171_94','AUCF-22A-165_1023'), default_value='AUCF-22A-181_86', size=(15, 1), key='megane')],
			...
			if isRun:  # 認識実行中なら
				...
				if len(faces) > 0:  # 顔を見つけたら以下を処理する
					for face in faces:  #全ての顔 faces から一つの face を取り出して
						...
						if (values['megane'] == 'AUCF-22A-165_1023'):
							icon, icon_w, icon_h = load_icon(IMAGE_PATH5, distance)
							if (pos != None) and (distance > 0.0):
								# icon画像の中心をランドマーク33番（鼻）に対応づける
								x = pos.x - int(icon_w/2)
								y = pos.y - int((icon_h*2)/3)
